...
В принципе это почти то же самое, что и предыдущее сопоставление. Только тут больше свободы для поиска соответствий. Например в примере ниже мы сделали букву ```о``` не обязательной для условий сопоставления. А вот буква ```g``` - обязателья, если ее не будет в проверямой строке, но будет ```o``` , тест выдаст ```null```.
