...
* ```.stop``` - прекращает дальнейшую передачу текущего события. В примере ниже мы использовали этот модификатор на теге ```span```. Теперь когда мышь будет находится в зоне это модификатора, функция по выводу координат мыши не будет отрабатывать.
* ```.enter``` - v-ov:keyup.enter будет отрабатывать функции при нажатии на Enter. Одна из разновидностей модификатора на события клавиатуры. Модификаторов на кейкоды много - более подробное в документации по [Vue](https://ru.vuejs.org/v2/guide/events.html#Модификаторы-клавиш)
...
<div id="idName">
    ...
    <input type="text" v-ov:keyup.enter.space="alertValue">
</div>
